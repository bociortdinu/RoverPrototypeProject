  ...
  if (command.length() > 0 && is_written == false )
  {
    ...
    if (command == "rFRONT")       
      ...
      if (command == "bLEFT")       
      {
        ...
        Wire.write("bLEFT");        // sends five bytes
        Wire.endTransmission();    // stop transmitting
      }
      if (command == "bRIGHT")       
      {
        Wire.beginTransmission(5); // transmit to device #5
        Wire.write("bRIGHT");        // sends five bytes
        Wire.endTransmission();    // stop transmitting
      }
      if (command == "bFRONT")       
      {
        Wire.beginTransmission(5); // transmit to device #5
        Wire.write("bFRONT");        // sends five bytes
        Wire.endTransmission();    // stop transmitting
      }
      if (command == "bBACK")       
      {
        Wire.beginTransmission(5); // transmit to device #5
        Wire.write("bBACK");        // sends five bytes
        Wire.endTransmission();    // stop transmitting
      }
      if (command == "bUP")       
      {
        Wire.beginTransmission(5); // transmit to device #5
        Wire.write("bUP");        // sends five bytes
        Wire.endTransmission();    // stop transmitting
      }
      if (command == "bDOWN")       
      {
        Wire.beginTransmission(5); // transmit to device #5
        Wire.write("bDOWN");        // sends five bytes
        Wire.endTransmission();    // stop transmitting
      }
    command = "";
